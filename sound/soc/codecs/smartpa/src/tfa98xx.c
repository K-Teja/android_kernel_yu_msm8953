...
void tfa98xx_deferred_calibration_status(Tfa98xx_handle_t handle, int calibrateDone)
{
	...
	pr_debug("wr_value: %d, calibrateDone: %d.\n", calib->wr_value, calibrateDone);
	if (calib->wr_value) {
						...
						struct snd_soc_dai *dai)
{
	...
	if (NULL == substream->runtime) {
		pr_warning("runtime is NULL, maybe offload, return 0.\n");
		return 0;
	}
	pr_info("\n");
...
static int tfa98xx_parse_dt(struct device *dev, struct tfa98xx *tfa98xx,
	...
	pr_info("%s, reset_gpio=%d, irq_gpio=%d", __func__, tfa98xx->reset_gpio, tfa98xx->irq_gpio);
	return 0;
}
...
retry:
		...
		if (ret < 0) {
			dev_err(&i2c->dev, "Failed to read Revision register: %d\n",
				ret);
			if (try ++ < 10) {
				dev_err(&i2c->dev, "Try %d times to read rev\n",try);
				goto retry;
			}
			return -EIO;
		}
