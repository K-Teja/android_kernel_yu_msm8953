...
static int calibration_retry = 0;
...
int tfa_calibrate(Tfa98xx_handle_t handle) {
	...
	/* check AREFS and CLKS: not ready if either is clear */
	if((TFA_GET_BF_VALUE(handle, AREFS, status) == 0)
			|| (TFA_GET_BF_VALUE(handle, CLKS, status) == 0)) {
		pr_debug("CLKS and PLLs is not stable yet!!!\n");
	}

	/* clear mtpex */
	error = tfa98xx_set_mtp(handle, 0, 0x2);
	if (error) {
		pr_err("set mtp failed, error: %d\n", error);
		return error ;
	}
...
{
	enum Tfa98xx_Error err = Tfa98xx_Error_Ok;
	int calibrateDone, spkr_count = 0, calibrateIme_Invalid = 0;
	...
	if(err != Tfa98xx_Error_Ok) {
		pr_err("Speaker calibration failed!, err: %d\n", err);
	...
	if(err == Tfa98xx_Error_Ok) {
		...
		if (spkr_count == 1) {
			pr_info("Speaker impedance is %d mOhms \n", handles_local[handle].mohm[0]);
		} else {
			pr_info(" Prim Speaker:%d mOhms,  Sec Speaker:%d mOhms\n",
						handles_local[handle].mohm[0], 
						handles_local[handle].mohm[1]);
		}
/* wt Should config base on speaker resistance (mOHM +-20%) */
#define imped_min 6400		//6.4 OHM
#define imped_max 9600		//9.6 OHM
		pr_info("Re25 range[%d ~ %d].\n", imped_min, imped_max);
		if(handles_local[handle].mohm[0] < imped_min || handles_local[handle].mohm[0] > imped_max){
			pr_err("Speaker impedance is invalid!\n");
			calibrateIme_Invalid = 1;
		}else{
			pr_info("Speaker calibration is successful! \n");
		}
	}
	...
	if (calibrateDone == 0){
		pr_err("Calibration is not done yet!\n");
		/* force cold boot */
		tfaRunColdboot(handle, 1); // set ACS
		err = Tfa98xx_Error_Other;
	}
	if(calibrateIme_Invalid){
		if(calibration_retry < 1){
			calibration_retry++;
			pr_err("Need to reset MtpEx and set ACS, for next time redo calibration!\n");
			tfa_calibrate(handle);
		}
		err = Tfa98xx_Error_Other;
	}
...
#define CF_CONTROL 0x8100
	...
	pr_debug("ACS = %d, state: %d.\n", TFA_GET_BF(handle, ACS), state);
	...
	while ( state != TFA_GET_BF(handle, ACS)) {
		...
		PRINT_ASSERT(err);
		msleep_interruptible(10); /* wait 20ms to avoid busload */
	...
	pr_debug("ACS = %d, tries = %d. \n", TFA_GET_BF(handle, ACS), tries);
...
enum Tfa98xx_Error tfaRunStartup(Tfa98xx_handle_t handle, int profile)
{
	...
	switch (tfa98xx_dev_revision(handle)) {
		case 0x80:
			pr_debug("Need to reset DSP here for tfa9890\n");
...
/*
 * recalibrate speaker impedance.
 */
enum tfa_error tfa_recalibrate(int next_profile, int *vstep)
{
	enum Tfa98xx_Error err = Tfa98xx_Error_Ok;
	int dev, devcount = tfa98xx_cnt_max_device();
	//int cal_profile = -1;
	uint16_t regvalue = 0xff;
	...
	for( dev=0; dev < devcount; dev++) {
		...
		if (tfa98xx_runtime_verbose)
			pr_debug("device [%s]\n", tfaContDeviceName(dev));
		err = tfaRunStartup(dev, next_profile);
		if ( err != Tfa98xx_Error_Ok) {
			continue;
		}
		/* reset profile to default value. */
		handles_local[dev].profile = -1;
		tfa98xx_read_register16(dev, 0x80, &regvalue);
		pr_debug("Get mtp[0x80]: 0x%04x.\n", regvalue);
		/* reset mtp and set ACS. */
	...
	/* start devices to do recalibration. */
	err = tfa_start(next_profile, vstep);
	if ( err != Tfa98xx_Error_Ok) {
		pr_err("tfa start failed, err: %d.\n", err);
	}
	/* stop devices. */
	tfa_stop();	/* if needed. */
