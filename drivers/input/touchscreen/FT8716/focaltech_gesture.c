...
//k80037 Customization

struct k80037_gesture_struct k80037_gesture_data={0,0};
...
static void fts_gesture_report(struct input_dev *input_dev,int gesture_id)
{
	...
	if (gesture_id == GESTURE_DOUBLECLICK){
	    ...
	    input_report_key(input_dev, K80037_DOUBLE_CLICK, 1);
	    input_sync(input_dev);
	    input_report_key(input_dev, K80037_DOUBLE_CLICK, 0);
	    input_sync(input_dev);
	}
    }
...
// k80037 gesture init
static ssize_t k80037_gesture_read(struct file *file, char __user * page, size_t size, loff_t * ppos)
{
    return 0;
}
static ssize_t k80037_gesture_write(struct file *filp, const char __user * buff, size_t len, loff_t * off)
{
    int ret;
    char temp[20]="";
	...
	k80037_gesture_data.gesture_all_switch =1;
    else 
	k80037_gesture_data.gesture_all_switch =0;
#else
    ret = kstrtouint(temp, 0, &k80037_gesture_data.gesture_all_switch);
    if (ret){
    ...
    FTS_DEBUG("%s k80037_gesture_data.gesture_all_switch :%d\n",__func__,k80037_gesture_data.gesture_all_switch);
    
    return len;
}
static const struct file_operations k80037_gesture_fops = {
         .owner = THIS_MODULE,
         .read = k80037_gesture_read,
         .write = k80037_gesture_write,
};

int k80037_gesture_init(struct input_dev *input_dev)
{
    struct proc_dir_entry *proc_entry = NULL;
    struct proc_dir_entry *parent;
     
    parent = proc_mkdir("touchscreen_feature", NULL);
    if (!parent) {
        pr_err("%s: failed to create proc entry\n", __func__);
        return -ENOMEM;
    }

    proc_entry = proc_create(K80037_GESTURE_NODE, 0666, NULL, &k80037_gesture_fops);
    if (proc_entry == NULL) {
	FTS_ERROR("CAN't create proc entry /proc/%s !", K80037_GESTURE_NODE);
	return -1;
    } else {
	FTS_DEBUG("Created proc entry /proc/%s !", K80037_GESTURE_NODE);
    }
    input_set_capability(input_dev, EV_KEY,K80037_DOUBLE_CLICK);
    __set_bit(K80037_DOUBLE_CLICK, input_dev->keybit);
