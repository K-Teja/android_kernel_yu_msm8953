...
*/
    ...
    input_set_capability(ts->input_dev, EV_KEY, KEY_GESTURE);//Modify,@wangdongbo.wt_20160602
    __set_bit(KEY_GESTURE, ts->input_dev->keybit);
...
//Add,@wangdongbo.wt_20160612
	...
	if (ret == SUCCESS)
	{
		GTP_DEBUG("IC Config Version: %d, 0x%02X",cfg_version, cfg_version);
	}

	if(tp_sensor_id==1)
	{
		sprintf(tp_string_version,"EACH,GT915L_%x,V%d",  version_info,cfg_version);
	}
	else
	{
		sprintf(tp_string_version,"XXX,GT915L_%x,V%d",  version_info,cfg_version);
	}
