...
static void aw2013_led_debug_brightness(struct aw2013_led *led,
...
if (led->cdev.brightness ==255) {
	...
	aw2013_write(led, 0x08, 0x00);
	aw2013_write(led, 0x11, 0xff);//blue
	aw2013_write(led, 0x1c, 0xff);
	aw2013_write(led, 0x1d, 0xff);
	aw2013_write(led, 0x1e, 0xff);
	aw2013_read(led, 0x07, &val);		
	...
	{    
	aw2013_write(led, 0x01, 0x01);
	aw2013_write(led, 0x03, 0x01);
	aw2013_write(led, 0x06, 0x00);
	aw2013_write(led, 0x08, 0x00);//00-independent,08-syns
	aw2013_write(led, 0x12, 0xff);//green
	aw2013_write(led, 0x1c, 0xff);
	aw2013_write(led, 0x1d, 0xff);
	aw2013_write(led, 0x1e, 0xff);
	aw2013_read(led, 0x07, &val);		
	aw2013_write(led, 0x07, (val|0x04));
	 #ifdef LED_DEBUG_AW2015
	 pr_err("line=%d,func=%s, brightness=%d\n",__LINE__,__func__,led->cdev.brightness);
	 #endif
	}
...
else  
{
	...
	aw2013_write(led, 0x10, 0xFF);//red
	aw2013_write(led, 0x1c, 0xff);
	aw2013_write(led, 0x30, 0x64);
	aw2013_write(led, 0x31, 0x6A);
	aw2013_write(led, 0x32, 0x08);
	...
	aw2013_write(led, 0x01, 0x03);
	aw2013_write(led, 0x03, 0x01);//cureent 6mA
	aw2013_write(led, 0x06, 0x01);
	aw2013_write(led, 0x08, 0x00);
	aw2013_read(led, 0x07, &val);		
	aw2013_write(led, 0x07, (val|0x04));
	aw2013_write(led, 0x12, 0xff);//green
	...
	aw2013_write(led, 0x3B, 0x69);
	aw2013_write(led, 0x3C, 0x08);
	aw2013_read(led, 0x09, &val);	
	aw2013_write(led, 0x09, (val|0x04));
	#ifdef LED_DEBUG_AW2015
	pr_err("line=%d,func=%s, brightness=%d\n",__LINE__,__func__,led->cdev.brightness);
     ...
     if(strcmp(led->cdev.name, "red") == 0)
       {
	    ...
	    pr_err("line=%d,func=%s, brightness=%d\n",__LINE__,__func__,led->cdev.brightness);
              ...
              aw2013_write(led, 0x08, 0x00);
	    aw2013_read(led, 0x07, &val);
             aw2013_write(led, 0x07, (val&0xfb));
         #ifdef LED_DEBUG_AW2015
	pr_err("line=%d,func=%s, brightness=%d\n",__LINE__,__func__,led->cdev.brightness);
	#endif
      }
...
static ssize_t aw2013_led_debug_show(struct device *dev,
...
static ssize_t aw2013_led_debug_store(struct device *dev,
			     ...
			     const char *buf, size_t len)
{
	...
	if (ret)
		return ret;
	mutex_lock(&led->pdata->led->lock);
	aw2013_led_debug_brightness(led, brightness);
	mutex_unlock(&led->pdata->led->lock);
...
static DEVICE_ATTR(led_debug, 0664, aw2013_led_debug_show, aw2013_led_debug_store);
			     ...
			     unsigned long  brightness)
{
                 u8 val=0;
		led->cdev.brightness = brightness ;
		aw2013_write(led, 0x08, 0x08);
		if (led->cdev.brightness > 0) {
			if (led->cdev.brightness > led->cdev.max_brightness)
				led->cdev.brightness = led->cdev.max_brightness;
				aw2013_write(led, 0x33, 1);
				aw2013_write(led, 0x03, 0x02);
				aw2013_write(led, 0x4, 0);//LED Operating Mode Select. 0:manual mode.
				aw2013_write(led, AW_REG_GLOBAL_CONTROL,AW_LED_MOUDLE_ENABLE_MASK);
				aw2013_write(led, 0x10 + led->id,led->cdev.brightness);//The LED1 output current value is IMAX * ILED1_y / 255.
				aw2013_read(led, 0x07, &val);
				...
				struct device_node *node)
{
	...
	for_each_child_of_node(node, temp) {
		...
		//led->cdev.brightness_set = aw2013_led_debug_brightness;
